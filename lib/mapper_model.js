...
_.extend(MapperModel.prototype, {
	...
	dependency: function (key) {
		this._dependencies[key] = this._dependencies[key] || new Tracker.Dependency();
		return this._dependencies[key];
	},

	get: function (key) {
		if (Meteor.isClient) this.dependency(key).depend();
		return _.extend({}, this._attributes, this._changedAttributes)[key];
	},

	getAttributes: function (key) {
		if (Meteor.isClient) this.dependency('_attributes').depend();
	...
	setAttributes: function (attributes) {
		...
		var result = _.extend({}, this._attributes, this._changedAttributes);
		if (Meteor.isClient) {
			if (!_.isEmpty(newChangedAttributes)) {
				this.dependency('_attributes').changed();
				_.each(newChangedAttributes, function (value, key) {
					this.dependency(key).changed();
				}, this);
			}
		}
		return result;
